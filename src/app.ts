 ...
 *
 * @returns Promise resolving to the configured Fastify application instance
 */
export async function build(): Promise<FastifyInstance> {
  ...
  app.get(
    ...
    (
      ...
      return reply.view('auth/error', { heading, message });
    },
  );
   ...
   *
   * @param request  Fastify Request object for the incoming HTTP request
   * @param reply  Fastify Reply object
   */
   ...
   *
   * @param request  - Fastify Request object (unused)
   * @param reply  - Fastify Reply object
   */
  app.get(
    '/auth/logout/success',
   ...
   *
   * @param request   - Fastify Request object containing the query parameter `reason`
   * @param reply   - Fastify Reply object
   */
  app.get(
    '/auth/logout/error',
    (
      ...
      return reply.view('auth/logout/error', { reason });
    },
  );

  /**
   * Mounts Auth.js Fastify plugin to handle OAuth 2.0/OIDC authentication flows.
   *
   ...
   *
   * The FastifyAuth plugin registers several endpoints for authentication:
   ...
   * plugin to prevent conflicts. Fastify matches routes in definition order,
   ...
   *
   * // ✓ Mount FastifyAuth AFTER custom routes
   * await app.register(FastifyAuth(authConfig), { prefix: '/auth' });
   * ```
   *
   * Incorrect Order (will cause UnknownAction errors):
   * ```typescript
   * // ✗ FastifyAuth intercepts everything first
   * await app.register(FastifyAuth(authConfig), { prefix: '/auth' });
   *
   ...
   *
   * @see {@link https://authjs.dev/reference/fastify} Auth.js Fastify documentation
   ...
   *
   * @param request  - Fastify Request object, guaranteed to have an authenticated session
   * @param reply  - Fastify Reply object
   */
  app.get(
    '/profile',
   ...
   *
   * @param request  - Fastify Request object for the incoming request
   * @param reply  - Fastify Reply object
   * @returns    void
   */
