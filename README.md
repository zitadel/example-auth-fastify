...
[Fastify](https://fastify.dev/) is a fast and low overhead web framework for Node.js. It works on Node.js environments including local development, production servers, Docker containers, and cloud platforms. In a modern setup, your Fastify application manages both your application's frontend and backend logic through server-side routes and plugins.

To secure such an application, you need a reliable way to handle user logins. For the Fastify ecosystem, [Auth.js](https://authjs.dev/) (formerly NextAuth.js) is the standard and recommended library for authentication. Think of it as a flexible security guard for your app. This guide demonstrates how to use Auth.js with a Fastify application to implement a secure login with ZITADEL.
...
This example uses **Auth.js**, the standard for Fastify authentication. While ZITADEL doesn't offer a specific SDK, Auth.js is highly modular. It works with a "provider" that handles the communication with ZITADEL. Under the hood, this example uses the powerful OIDC standard to manage the secure PKCE flow.
...
### 1. Security headers (Fastify plugins)

**Partially enabled.** Fastify includes some security headers by default, but consider adding custom headers in your application:
